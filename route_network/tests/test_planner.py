...
def makeSeg(start, end, oneway=False):
    ...
    seg = RouteSegment(id=uu, start=UniqueID(uuid=start), end=UniqueID(uuid=end))
    if oneway:
...
def makeRequest(network, start, goal):
    return GetRoutePlanRequest(
        UniqueID(uuid=network), UniqueID(uuid=start), UniqueID(uuid=goal)
    )


def makeSegment(id, start, end):
    return RouteSegment(id=UniqueID(id), start=UniqueID(start), end=UniqueID(end))
...
def tiny_graph():
    ...
    r = RouteNetwork()
    r.points.append(
        makeWayPoint("da7c242f-2efe-5175-9961-49cc621b80b9", 30.3840168, -97.72821)
    )
    r.points.append(
        makeWayPoint("812f1c08-a34b-5a21-92b9-18b2b0cf4950", 30.385729, -97.7316754)
    )
    r.segments.append(
        makeSegment(
...
def triangle_graph():
    ...
    r.points.append(
        makeWayPoint("2b093523-3b39-5c48-a11c-f7c65650c581", 30.3849643, -97.7269564)
    )
    s = makeSegment(
        "41b7d2be-3c37-5a78-a7ac-248d939af379",
        "da7c242f-2efe-5175-9961-49cc621b80b9",
...
class TestPlanner(unittest.TestCase):
    ...
    def test_tiny_route_network(self):
        ...
        self.assertEqual(path.segments[0].uuid, "41b7d2be-3c37-5a78-a7ac-248d939af379")
        path = pl.planner(
            makeRequest(
                pl.graph.id.uuid,
                "812f1c08-a34b-5a21-92b9-18b2b0cf4950",
        ...
        self.assertEqual(len(path.segments), 1)
        self.assertEqual(path.segments[0].uuid, "2df38f2c-202b-5ba5-be73-c6498cb4aafe")
    ...
    def test_bogus_start_point(self):
        pl = Planner(tiny_graph())
        req = makeRequest(
            pl.graph.id.uuid, "deadbeef", "da7c242f-2efe-5175-9961-49cc621b80b9"
        )
        self.assertRaises(ValueError, pl.planner, req)
    ...
    def test_bogus_goal(self):
        pl = Planner(tiny_graph())
        req = makeRequest(
            pl.graph.id.uuid, "812f1c08-a34b-5a21-92b9-18b2b0cf4950", "deadbeef"
        )  # invalid way point
        self.assertRaises(ValueError, pl.planner, req)
    ...
    def test_triangle_routes(self):
        ...
        self.assertEqual(path.segments[0].uuid, "2df38f2c-202b-5ba5-be73-c6498cb4aafe")
        self.assertEqual(path.segments[1].uuid, "8f2c2df3-be73-5ba5-202b-cb4aafec6498")
    ...
    def test_2x2_grid(self):
        ...
        for pt1 in g.points:
            for pt2 in g.points:
                path = pl.planner(makeRequest(g.id.uuid, pt1.id.uuid, pt2.id.uuid))
